...
api_input = st.sidebar.text_input("Gemini API Key", type="password", value=API_KEY)
    ...
    if st.button("AI로 내역 기록하기"):
        if not user_input:
            ...
            with st.spinner(f"AI가 {current_country} 내역을 분석 중입니다..."):
                parsed_data = parse_expense_with_ai(user_input, current_country, selected_date_str, api_input)
